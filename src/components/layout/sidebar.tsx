    ...
    <>
      ...
      <div className={`flex items-center gap-3 px-4 py-4 border-b border-slate-200 dark:border-slate-800 ${collapsed ? "justify-center" : ""}`}>
        ...
        {!collapsed && (
          <div>
            ...
            <div className="text-xs text-black dark:text-black">AI Suite</div>
          </div>
        )}
        {!collapsed && (
          <button
            onClick={() => setCollapsed(true)}
            className="ml-auto text-black hover:text-black dark:hover:text-slate-300 transition-colors"
          >
      ...
      {collapsed && (
        <button
          ...
          className="p-2 text-black hover:text-black dark:hover:text-slate-300 transition-colors flex justify-center mt-1"
        >
          ...
          <div key={section.title}>
            {!collapsed && (
              <button
                ...
                className="flex items-center justify-between w-full px-2 py-1.5 text-xs font-bold text-black dark:text-black uppercase tracking-wider hover:text-slate-900 dark:hover:text-slate-200 transition-colors"
              >
                {section.title}
                <ChevronDown
        ...
        {collapsed ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-black dark:text-black truncate">
                {(session?.user as any)?.role || "Employee"}
              </div>
            </div>
            <button
              onClick={() => signOut({ callbackUrl: "/login" })}
              className="text-black hover:text-black dark:hover:text-slate-300 transition-colors"
              title="Logout"
            >
